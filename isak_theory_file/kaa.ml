module K : sig
  ...
  type 'a metaVar
  type char
  type ('a, 'b, 'c) suBigKWithBag
  type ('a, 'b, 'c) suB
  type ('a, 'b, 'c) suBigKWithLabel
  type ('a, 'b, 'c) suKl
  type ('a, 'b, 'c) suKLabel
  type ('a, 'b, 'c) suKItem
  type ('a, 'b, 'c) suKFactor
  type ('a, 'b, 'c) suL
  type ('a, 'b, 'c) suM
  type ('a, 'b, 'c) suS
...
type reg = Eps | Sym of char | Alt of reg * reg | Seqa of reg * reg |
  Rep of reg;;

let rec equal_reg
  x0 x1 = match x0, x1 with Seqa (x41, x42), Rep x5 -> false
    | Rep x5, Seqa (x41, x42) -> false
    | Alt (x31, x32), Rep x5 -> false
    | Rep x5, Alt (x31, x32) -> false
    | Alt (x31, x32), Seqa (x41, x42) -> false
    | Seqa (x41, x42), Alt (x31, x32) -> false
    | Sym x2, Rep x5 -> false
    | Rep x5, Sym x2 -> false
    | Sym x2, Seqa (x41, x42) -> false
    | Seqa (x41, x42), Sym x2 -> false
    | Sym x2, Alt (x31, x32) -> false
    | Alt (x31, x32), Sym x2 -> false
    | Eps, Rep x5 -> false
    | Rep x5, Eps -> false
    | Eps, Seqa (x41, x42) -> false
    | Seqa (x41, x42), Eps -> false
    | Eps, Alt (x31, x32) -> false
    | Alt (x31, x32), Eps -> false
    | Eps, Sym x2 -> false
    | Sym x2, Eps -> false
    | Rep x5, Rep y5 -> equal_reg x5 y5
    | Seqa (x41, x42), Seqa (y41, y42) -> equal_reg x41 y41 && equal_reg x42 y42
    | Alt (x31, x32), Alt (y31, y32) -> equal_reg x31 y31 && equal_reg x32 y32
    | Sym x2, Sym y2 -> equal_chara x2 y2
    | Eps, Eps -> true;;

type 'a synAttrib = Strict of nat list | Seqstrict | Left | Right | Hook of 'a |
  Function | Klabel of char list | Bracket | Tokena | Avoid | OnlyLabel |
  Regex of reg | NonAssoc;;

type ('a, 'b) kSyntax = Syntax of 'a sort * 'a symbol nelist * 'b synAttrib list
  | Subsort of 'a sort * 'a sort | Token of 'a sort * reg * 'b synAttrib list |
  ListSyntax of 'a sort * 'a sort * char list * 'b synAttrib list;;

let rec equal_synAttriba _A
  x0 x1 = match x0, x1 with Regex x12, NonAssoc -> false
    | NonAssoc, Regex x12 -> false
    | OnlyLabel, NonAssoc -> false
    | NonAssoc, OnlyLabel -> false
    | OnlyLabel, Regex x12 -> false
    | Regex x12, OnlyLabel -> false
    | Avoid, NonAssoc -> false
    | NonAssoc, Avoid -> false
    | Avoid, Regex x12 -> false
    | Regex x12, Avoid -> false
    | Avoid, OnlyLabel -> false
    | OnlyLabel, Avoid -> false
    | Tokena, NonAssoc -> false
    | NonAssoc, Tokena -> false
    | Tokena, Regex x12 -> false
    | Regex x12, Tokena -> false
    ...
    | Bracket, Regex x12 -> false
    | Regex x12, Bracket -> false
    ...
    | Klabel x7, Regex x12 -> false
    | Regex x12, Klabel x7 -> false
    ...
    | Function, Regex x12 -> false
    | Regex x12, Function -> false
    ...
    | Hook x5, Regex x12 -> false
    | Regex x12, Hook x5 -> false
    ...
    | Right, Regex x12 -> false
    | Regex x12, Right -> false
    ...
    | Left, NonAssoc -> false
    | NonAssoc, Left -> false
    | Left, Regex x12 -> false
    | Regex x12, Left -> false
    ...
    | Seqstrict, Regex x12 -> false
    | Regex x12, Seqstrict -> false
    ...
    | Strict x1, Regex x12 -> false
    | Regex x12, Strict x1 -> false
    ...
    | Regex x12, Regex y12 -> equal_reg x12 y12
    | Klabel x7, Klabel y7 -> equal_lista equal_char x7 y7
    | Hook x5, Hook y5 -> eq _A x5 y5
...
let rec equal_synAttrib _A =
  ({equal = equal_synAttriba _A} : 'a synAttrib equal);;

let rec equal_kSyntaxa _A _B
  x0 x1 = match x0, x1 with
        ...
        equal_sorta _A x41 y41 &&
          (equal_sorta _A x42 y42 &&
            (equal_lista equal_char x43 y43 &&
              equal_lista (equal_synAttrib _B) x44 y44))
    | Token (x31, x32, x33), Token (y31, y32, y33) ->
        equal_sorta _A x31 y31 &&
          (equal_reg x32 y32 && equal_lista (equal_synAttrib _B) x33 y33)
    ...
    | Syntax (x11, x12, x13), Syntax (y11, y12, y13) ->
        equal_sorta _A x11 y11 &&
          (equal_nelist (equal_symbol _A) x12 y12 &&
            equal_lista (equal_synAttrib _B) x13 y13);;

let rec equal_kSyntax _A _B =
  ({equal = equal_kSyntaxa _A _B} : ('a, 'b) kSyntax equal);;
...
type ('a, 'b, 'c, 'd) kModuleItem = TheSyntax of ('a, 'b) kSyntax |
  Imports of 'c | TheConfiguration of ('a, 'b, 'd) bag |
  TheRule of ('a, 'b, 'd) kRule;;
...
let rec equal_kModuleItema _A _B _C _D
  x0 x1 = match x0, x1 with TheConfiguration x3, TheRule x4 -> false
    ...
    | TheSyntax x1, TheSyntax y1 -> equal_kSyntaxa _A _B x1 y1;;
...
let rec syntaxToKItem _B
  = function
    Syntax (a, b, c) ->
      (if isBracket c then None
        else (match getKLabelName c
               with None ->
                 (if membera (equal_synAttrib _B) c Function
...
(getRidStrictAttrs c, true))))]
    ...
    | Token (a, s, c) ->
        Some [([a], ([], (SetTon
                           ...
                           (getRidStrictAttrs c,
                             membera (equal_synAttrib _B) c Function))))]
    | Subsort (a, b) -> None
    | ListSyntax (a, b, s, c) ->
...
let rec syntaxSetToKItemSetAux _B
  x0 order = match x0, order with [], order -> Some []
    | a :: l, order ->
        (match syntaxToKItem _B a with None -> None
          | Some la ->
            (match syntaxSetToKItemSetAux _B l (order @ [a]) with None -> None
              | Some laa -> Some (la @ laa)));;

let rec inserta _A x xs = (if membera _A xs x then xs else x :: xs);;

let rec getAllSyntaxInKModuleItemList _A _B
  = function [] -> []
    | TheSyntax x :: l ->
        inserta (equal_kSyntax _A _B) x (getAllSyntaxInKModuleItemList _A _B l)
    | Imports x :: l -> getAllSyntaxInKModuleItemList _A _B l
    | TheConfiguration x :: l -> getAllSyntaxInKModuleItemList _A _B l
    | TheRule x :: l -> getAllSyntaxInKModuleItemList _A _B l;;

let rec insertAll _A a x1 = match a, x1 with a, [] -> a
                       | a, b :: l -> insertAll _A (inserta _A b a) l;;

let rec getAllSyntaxInKFile _A _B
  = function TheFile (Single (TheRequires m)) -> []
    | TheFile (Single (TheModule (Module (a, m)))) ->
        getAllSyntaxInKModuleItemList _A _B m
    | TheFile (Con (TheRequires m, xs)) ->
        getAllSyntaxInKFile _A _B (TheFile xs)
    | TheFile (Con (TheModule (Module (a, m)), xs)) ->
        insertAll (equal_kSyntax _A _B) (getAllSyntaxInKModuleItemList _A _B m)
          (getAllSyntaxInKFile _A _B (TheFile xs));;

let rec syntaxSetToKItemTest _A _B
  theory = syntaxSetToKItemSetAux _B (getAllSyntaxInKFile _A _B theory) [];;
...
let rec genStrictRules _B _C
  s x1 database subG = match s, x1, database, subG with
    s, [], database, subG -> Some []
    | s, b :: l, database, subG ->
        (let (ty, (tyl, (SingleTon sa, (stl, true)))) = b in
          (if membera (equal_synAttrib _C) stl Seqstrict
            then (if is_none (getStrictInAttributes stl)
                   then (match
                          ...
                          with None -> None
                          | Some rl ->
                            (match genStrictRules _B _C s l database subG
                              with None -> None | Some rla -> Some (rl @ rla)))
                   else None)
            else (match getStrictList stl tyl
                   with [] -> genStrictRules _B _C s l database subG
                   | n :: nl ->
                     (match
...
let rec strictRuleTest _B _C
  vars theory subG =
    (match syntaxSetToKItemTest _B _C theory with None -> None
      | Some database ->
        (match genStrictRules _B _C vars database database subG
          with None -> None | Some a -> Some a));;
...
let rec validSyntaxSort _B _C
  vars theory database subG =
    validSyntaxs _B database &&
      (validArgSynax _B database &&
        (not (is_none (syntaxSetToKItemTest _B _C theory)) &&
          not (is_none (strictRuleTest _B _C vars theory subG))));;
...
let rec preSubsortGraph _A _B
  theory =
    formGraph (equal_sort _A)
      (insertAll (equal_sort _A)
        (getAllSorts _A (getAllSyntaxInKFile _A _B theory)) builtinSorts)
      (preAllSubsorts _A _B theory);;

let rec kResultSubsorts _A _B
  theory =
    getKResultSubsorts _A (getAllSorts _A (getAllSyntaxInKFile _A _B theory))
      (preSubsortGraph _A _B theory);;

let rec allSubsorts _A _B
  theory =
    getAllSubsortInKFile _A theory @
      addImplicitSubsorts (equal_sort _A) KItem builtinSorts
        (getAllSorts _A (getAllSyntaxInKFile _A _B theory)) @
        [(KResult, KItem); (KItem, K)] @
          topSubsort @ kResultSubsorts _A _B theory;;

let rec subsortGraph _A _B
  theory =
    formGraph (equal_sort _A)
      (insertAll (equal_sort _A)
        (getAllSorts _A (getAllSyntaxInKFile _A _B theory)) builtinSorts)
      (allSubsorts _A _B theory);;

let rec invalidSortChecks _A _B
  theory =
    not (hasInvalidSubsort _A (getAllSubsortInKFile _A theory)) &&
      not (hasInvalidTranstiveClosure (equal_sort _A)
            (subsortGraph _A _B theory) (subsortGraph _A _B theory));;

let rec syntaxSetToKItemSet _A _B
  theory =
    (match syntaxSetToKItemTest _A _B theory with None -> [] | Some l -> l);;
...
let rec uniqueKLabelSyntax _A _B
  theory = isUnitLabel (equal_label _A) (syntaxSetToKItemSet _A _B theory);;
...
let rec kcompile _A _B _D
  theory =
    (match syntaxSetToKItemTest _A _B theory
      with None ->
        Failure
          [Char (Nibble4, NibbleE); Char (Nibble6, NibbleF);
            ...
            Char (Nibble2, NibbleE)]
      | Some database ->
        (if invalidSortChecks _A _B theory
          then (if uniqueKLabelSyntax _A _B theory
                 then (if validSyntaxSort _A _B [] theory database
                            (subsortGraph _A _B theory)
                        then (match
 ...
 Failure
   [Char (Nibble5, Nibble4); Char (Nibble6, Nibble8); Char (Nibble6, Nibble5);
     ...
     Char (Nibble7, Nibble3); Char (Nibble2, NibbleE)]
                                       | Some stl ->
 Success
   (theory, (database, (subsortGraph _A _B theory, (confi, allRules @ stl)))))))
                        else Failure
                               [Char (Nibble4, NibbleB);
                                 Char (Nibble2, Nibble0);
                                 Char (Nibble7, Nibble4);
