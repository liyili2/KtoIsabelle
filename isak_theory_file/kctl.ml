module Kctl : sig
  ...
  type ('a, 'b, 'c) irSet
  type ('a, 'b, 'c) irList
  type ('a, 'b, 'c) irBigKWithBag
  type ('a, 'b, 'c) irBag
  type ('a, 'b, 'c) irBigKWithLabel
  type ('a, 'b, 'c) irKList
  type ('a, 'b, 'c) irKItem
  type ('a, 'b, 'c) subsFactor
  type 'a ctlForm
  type 'a seq
  type 'a pred
  type ('a, 'b, 'c, 'd) kFile
  type ('a, 'b) oneStep
  val kctl :
    'a equal -> 'b equal -> 'c equal -> 'd equal -> 'e equal ->
      ('a, 'b, 'c, 'd) kFile ->
        nat ->
          ('d metaVar * ('a, 'b, 'd) subsFactor) list ->
            'b var list ->
              (('b var * ('a, 'b, 'e) irBigKWithBag) list) ctlForm ->
                (bool, (char list)) oneStep -> bool
end = struct
...
let rec minus_nata m n = match m, n with Suc m, Suc n -> minus_nata m n
                     | Zero_nat, n -> Zero_nat
                     | m, Zero_nat -> m;;
...
and equal_suBigKWithBaga _A _B _C
    ...
    | ItemB (x11, x12, x13), ItemB (y11, y12, y13) ->
        equal_vara _B x11 y11 &&
          (equal_lista equal_feature x12 y12 &&
            equal_suBigKWithBaga _A _B _C x13 y13)
and equal_suBigKWithLabel _A _B _C
  x0 x1 = match x0, x1 with SUBigBag x1, SUBigLabel x2 -> false
    ...
    | SUBigBag x1, SUBigBag y1 -> equal_suBigKWithBaga _A _B _C x1 y1
...
let rec equal_suBigKWithBag _A _B _C =
  ({equal = equal_suBigKWithBaga _A _B _C} : ('a, 'b, 'c) suBigKWithBag equal);;

type 'a set = Set of 'a list | Coset of 'a list;;

type 'a ctlForm = TrueCtl | PredCtl of 'a | NotCtl of 'a ctlForm |
  AndCtl of 'a ctlForm * 'a ctlForm | EXCtl of 'a ctlForm | EACtl of 'a ctlForm
  | EUCtl of 'a ctlForm * 'a ctlForm;;
...
let rec insertBagToList _A _B _C
  subG b x2 = match subG, b, x2 with subG, b, [] -> [b]
    | subG, b, a :: l ->
        (match syntacticMonoidInSUBag _A _B _C b a subG
          with None -> a :: insertBagToList _A _B _C subG b l
          | Some aa -> aa :: l);;

let rec insertPairToGraph _A _B _C _D _E
  subG x y xa3 = match subG, x, y, xa3 with subG, x, y, [] -> [(x, [y])]
    | subG, x, y, (a, al) :: l ->
        (match syntacticMonoidInSUBag _A _B _C x a subG
          with None -> (a, al) :: insertPairToGraph _A _B _C _D _E subG x y l
          | Some aa -> (aa, insertBagToList _A _D _E subG y al) :: l);;

let rec createEntry _A _B _C
  subG x xa2 = match subG, x, xa2 with subG, x, [] -> [(x, [])]
    | subG, x, (a, al) :: l ->
        (match syntacticMonoidInSUBag _A _B _C x a subG
          with None -> (a, al) :: createEntry _A _B _C subG x l
          | Some aa -> (aa, al) :: l);;

let rec updatePairInGraph _A _B _C _D _E
  subG x y g =
    insertPairToGraph _A _B _C _D _E subG x y (createEntry _A _B _C subG x g);;

let rec updateAllPairsInGraph _A _B _C _D _E
  subG x xa2 g = match subG, x, xa2, g with subG, x, [], g -> g
    | subG, x, y :: l, g ->
        updateAllPairsInGraph _A _B _C _D _E subG x l
          (updatePairInGraph _A _B _C _D _E subG x y g);;
...
let rec oneTransKCtl_i_i_i_i_i_i_i_i_i_o _A _B _C
  x xa xb xc xd xe xf xg xh =
    sup_pred
      (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, xh)))))))))
        (fun a ->
          (match a with (_, (_, (_, (_, (_, (_, (_, (gl, [])))))))) -> single gl
              ...
              | (allFunRules,
                  (database,
                    (subG,
                      (allKRules,
                        (allBagRules,
                          (transKRules, (transBagRules, (gl, b :: l))))))))
                -> bind (oneTransKCtl_i_i_i_i_i_i_i_i_i_o _A _B _C allFunRules
...
(equal_lista (equal_suB _A _B _C) b ba)))
                                 (fun () ->
                                   single
                                     (updatePairInGraph _A _B _C _B _C subG b ba
                                       xi))
                             | Div _ -> bot_pred))))))
        (sup_pred
          (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, xh)))))))))
              ...
              (match a
                ...
                | (allFunRules,
                  ...
                  -> bind (oneStepBagRule_i_i_i_i_i_i _A _B _C allFunRules
                       ...
                       (fun () ->
                         bind (oneTransKCtl_i_i_i_i_i_i_i_i_i_o _A _B _C
                                allFunRules database subG allKRules allBagRules
                                transKRules transBagRules gl l)
                           (fun xi ->
 ...
 bind (eq_i_o (replaceKCellsInList _A _B _C b xaa))
   (fun xab ->
     single (updateAllPairsInGraph _A _B _C _B _C subG b xab xi))))))))))
          (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, xh)))))))))
            (fun a ->
              (match a
                with (_, (_, (_, (_, (_, (_, (_, (_, [])))))))) -> bot_pred
 ...
 bind (oneTransBagRule_i_i_i_i_i_o _A _B _C allFunRules database subG
        transBagRules b)
   (fun xaa ->
     single (updateAllPairsInGraph _A _B _C _B _C subG b xaa xi))))))))))));;

let rec oneTransKCtl_i_i_i_i_i_i_i_i_i_i _A _B _C
  x xa xb xc xd xe xf xg xh xi =
    sup_pred
      (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, (xh, xi))))))))))
        (fun a ->
          (match a
            with (_, (_, (_, (_, (_, (_, (_, (gl, ([], gla))))))))) ->
              (if equal_lista
                    (equal_prod (equal_list (equal_suB _A _B _C))
                      (equal_list (equal_list (equal_suB _A _B _C))))
                    gl gla
                then single () else bot_pred)
            | (_, (_, (_, (_, (_, (_, (_, (_, (_ :: _, _))))))))) -> bot_pred)))
      (sup_pred
        (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, (xh, xi))))))))))
          (fun a ->
            (match a
              with (_, (_, (_, (_, (_, (_, (_, (_, ([], _))))))))) -> bot_pred
              | (allFunRules,
                  (database,
                    (subG,
                      (allKRules,
...
(equal_lista (equal_suB _A _B _C) b ba)))
 ...
 (equal_prod (equal_list (equal_suB _A _B _C))
   (equal_list (equal_list (equal_suB _A _B _C))))
 xj (updatePairInGraph _A _B _C _B _C subG b ba xaa)
                                     then single () else bot_pred))
                             | Div _ -> bot_pred))))))
        (sup_pred
          (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, (xh, xi))))))))))
 ...
 bind (eq_i_o (replaceKCellsInList _A _B _C b xaaa))
   (fun xaab ->
     (if equal_lista
           (equal_prod (equal_list (equal_suB _A _B _C))
             (equal_list (equal_list (equal_suB _A _B _C))))
           xj (updateAllPairsInGraph _A _B _C _B _C subG b xaab xaa)
       then single () else bot_pred))))))))))
          (bind (single (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, (xh, xi))))))))))
            (fun a ->
              (match a
 ...
 bind (oneTransBagRule_i_i_i_i_i_o _A _B _C allFunRules database subG
   ...
   (fun xaaa ->
     (if equal_lista
           (equal_prod (equal_list (equal_suB _A _B _C))
             (equal_list (equal_list (equal_suB _A _B _C))))
           xj (updateAllPairsInGraph _A _B _C _B _C subG b xaaa xaa)
       then single () else bot_pred))))))))))));;
...
let rec kctlAux_i_i_i_i_i_i_i_i_i_i_i_o _A _C _D
                 ...
                 (subG,
                   (_, (allFunRules,
                         (allAnywheres,
                           (allKRules,
                             (allBagRules,
                               (transKRules, (transBagRules, (gl, bl))))))))))
            -> bind (allAllFunAnywhere_i_i_i_i_i_o _A _C _D allFunRules
                      allAnywheres database subG bl)
                 (fun xk ->
                   bind (oneTransKCtl_i_i_i_i_i_i_i_i_i_i _A _C _D allFunRules
                          database subG allKRules allBagRules transKRules
                          transBagRules gl xk gl)
                     (fun () -> single gl))))
        (bind (single
                (x, (xa, (xb, (xc, (xd, (xe, (xf, (xg, (xh, (xi, xj)))))))))))
          (fun (database,
            ...
            -> bind (if_pred (less_nat Zero_nat n))
                 (fun () ->
                   bind (allAllFunAnywhere_i_i_i_i_i_o _A _C _D allFunRules
                          allAnywheres database subG bl)
                     (fun xk ->
                       bind (oneTransKCtl_i_i_i_i_i_i_i_i_i_o _A _C _D
                              allFunRules database subG allKRules allBagRules
                              transKRules transBagRules gl xk)
                         (fun xaa ->
                           bind (if_pred
                                  (not (equal_lista
 (equal_prod (equal_list (equal_suB _A _C _D))
   (equal_list (equal_list (equal_suB _A _C _D))))
 gl xaa)))
                             (fun () ->
                               bind (kctlAux_i_i_i_i_i_i_i_i_i_i_i_o _A _C _D
                                      database subG (minus_nata n one_nata)
                                      allFunRules allAnywheres allKRules
                                      allBagRules transKRules transBagRules xaa
                                      xk)
                                 single)))))));;
...
let rec findCellByName _A
  s x1 = match s, x1 with s, [] -> []
    | s, a :: l -> findCellByNameInSUB _A s a @ findCellByName _A s l
and findCellByNameInSUBigKWithBag _A s x1 = match s, x1 with s, SUK a -> []
                                       | s, SUList a -> []
                                       | s, SUSet a -> []
                                       | s, SUMap a -> []
                                       | s, SUBag a -> findCellByName _A s a
and findCellByNameInSUB _A
  s x1 = match s, x1 with
    s, ItemB (x, y, z) ->
      (if equal_vara _A s x then [z] else findCellByNameInSUBigKWithBag _A s z)
    | s, IdB x -> [];;

let rec findCellByNameList _A
  x0 b = match x0, b with [], b -> []
    | s :: l, b -> (s, findCellByName _A s b) :: findCellByNameList _A l b;;

let rec fst (x1, x2) = x1;;

let rec divmod_nat
  m n = (if equal_nata n Zero_nat || less_nat m n then (Zero_nat, m)
          else (let a = divmod_nat (minus_nata m n) n in
                let (q, aa) = a in
                 (Suc q, aa)));;
...
let rec getCaresList _A
  sl x1 = match sl, x1 with sl, [] -> []
    | sl, b :: l -> findCellByNameList _A sl b :: getCaresList _A sl l;;

let rec getCaresGraph _A
  sl x1 = match sl, x1 with sl, [] -> []
    | sl, (b, bl) :: l ->
        (findCellByNameList _A sl b, getCaresList _A sl bl) ::
          getCaresGraph _A sl l;;

let rec existInstance _A
  x0 g = match x0, g with [], g -> false
    | a :: l, g -> (if membera _A g a then true else existInstance _A l g);;

let rec collectNextTrueState _B
  x0 g = match x0, g with [], g -> []
    | (a, bl) :: l, g ->
        (if existInstance _B bl g then a :: collectNextTrueState _B l g
          else collectNextTrueState _B l g);;

let rec findPatternInCell _A _B _C _D
  subG a x2 acc = match subG, a, x2, acc with subG, a, [], acc -> []
    | subG, a, b :: l, acc ->
        (match patternMacroingInSUBigKWithBag _A _B _C _D a b acc subG
          with None -> findPatternInCell _A _B _C _D subG a l acc
          | Some acca -> acca :: findPatternInCell _A _B _C _D subG a l acc);;

let rec findPatternInCellWithList _A _B _C _D
  subG a s x3 = match subG, a, s, x3 with subG, a, s, [] -> []
    | subG, a, s, acc :: l ->
        findPatternInCell _A _B _C _D subG a s acc @
          findPatternInCellWithList _A _B _C _D subG a s l;;

let rec patternMatchInCells _A _B _C _D _E
  subG x1 g accs = match subG, x1, g, accs with subG, [], g, accs -> true
    | subG, (s, a) :: l, g, accs ->
        (match getValueTerm _B s g with None -> false
          | Some bl ->
            patternMatchInCells _A _B _C _D _E subG l g
              (findPatternInCellWithList _A _C _D _E subG a bl accs));;

let rec ctlStateCheck _A _B _C _D _E
  subG l g =
    (match l with [] -> true
      | (s, a) :: la ->
        (match getValueTerm _B s g with None -> false
          | Some bl ->
            patternMatchInCells _A _B _C _D _E subG la g
              (findPatternInCell _A _C _D _E subG a bl [])));;

let rec ctlStateCheckInList _A _B _C _D _E
  subG l x2 = match subG, l, x2 with subG, l, [] -> []
    | subG, l, a :: al ->
        (if ctlStateCheck _A _B _C _D _E subG l a
          then a :: ctlStateCheckInList _A _B _C _D _E subG l al
          else ctlStateCheckInList _A _B _C _D _E subG l al);;

let rec edgeNumsInGraph
  = function [] -> []
    | (a, b) :: l -> (a, size_list b) :: edgeNumsInGraph l;;

let rec intersetList _A
  a x1 = match a, x1 with a, [] -> []
    | aa, a :: l ->
        (if membera _A aa a then a :: intersetList _A aa l
          else intersetList _A aa l);;

let rec updateNumGraph _A (_B1, _B2)
  x0 a = match x0, a with [], a -> []
    | (a, n) :: l, b ->
        (if eq _A a b then (a, minus _B1 n (one _B2)) :: l
          else (a, n) :: updateNumGraph _A (_B1, _B2) l b);;
...
let rec minusList _A s x1 = match s, x1 with s, [] -> s
                       | s, a :: l -> minusList _A (removeAll _A a s) l;;

let rec eaCheckFactor _A (_B1, _B2, _B3, _B4)
  cs e t x3 = match cs, e, t, x3 with cs, e, t, [] -> (cs, (e, t))
    | cs, e, t, a :: l ->
        (match getValueTerm _A a cs
          with None ->
            eaCheckFactor _A (_B1, _B2, _B3, _B4) cs (inserta _A a e)
              (minusList _A t [a]) l
          | Some n ->
            (if eq _B4 n (zero _B3)
              then eaCheckFactor _A (_B1, _B2, _B3, _B4) cs (inserta _A a e)
                     (minusList _A t [a]) l
              else (if membera _A t a
                     then eaCheckFactor _A (_B1, _B2, _B3, _B4)
                            (updateNumGraph _A (_B1, _B2) cs a) e t l
                     else eaCheckFactor _A (_B1, _B2, _B3, _B4) cs e t l)));;

let rec eaCheckAux _A (_B1, _B2, _B3, _B4)
  cs pg x2 t = match cs, pg, x2, t with cs, pg, [], t -> t
    | cs, pg, a :: l, t ->
        (match getValueTerm _A a pg
          with None -> eaCheckAux _A (_B1, _B2, _B3, _B4) cs pg l t
          | Some e ->
            (let (cSa, aa) = eaCheckFactor _A (_B1, _B2, _B3, _B4) cs l t e in
             let (ab, b) = aa in
              eaCheckAux _A (_B1, _B2, _B3, _B4) cSa pg ab b));;

let rec euCheckFactor _A
  ls e t x3 = match ls, e, t, x3 with ls, e, t, [] -> (e, t)
    | ls, e, t, a :: l ->
        (if membera _A ls a && not (membera _A t a)
          then euCheckFactor _A ls (inserta _A a e) (inserta _A a t) l
          else euCheckFactor _A ls e t l);;

let rec euCheck _A
  pg ls x2 t = match pg, ls, x2, t with pg, ls, [], t -> t
    | pg, ls, a :: l, t ->
        (match getValueTerm _A a pg with None -> euCheck _A pg ls l t
          | Some al ->
            (let (aa, b) = euCheckFactor _A ls l t al in
              euCheck _A pg ls aa b));;

let rec ctlModelCheck _A _B _C _D _E
  subG vs gl pg x4 = match subG, vs, gl, pg, x4 with
    subG, vs, gl, pg, TrueCtl -> vs
    | subG, vs, gl, pg, PredCtl l ->
        ctlStateCheckInList _A _B _C _E _D subG l vs
    | subG, vs, gl, pg, NotCtl f ->
        minusList
          (equal_list
            (equal_prod _B (equal_list (equal_suBigKWithBag _A _C _D))))
          vs (ctlModelCheck _A _B _C _D _E subG vs gl pg f)
    | subG, vs, gl, pg, AndCtl (a, b) ->
        intersetList
          (equal_list
            (equal_prod _B (equal_list (equal_suBigKWithBag _A _C _D))))
          (ctlModelCheck _A _B _C _D _E subG vs gl pg a)
          (ctlModelCheck _A _B _C _D _E subG vs gl pg b)
    | subG, vs, gl, pg, EXCtl f ->
        collectNextTrueState
          (equal_list
            (equal_prod _B (equal_list (equal_suBigKWithBag _A _C _D))))
          gl (ctlModelCheck _A _B _C _D _E subG vs gl pg f)
    | subG, vs, gl, pg, EUCtl (a, b) ->
        euCheck
          (equal_list
            (equal_prod _B (equal_list (equal_suBigKWithBag _A _C _D))))
          pg (ctlModelCheck _A _B _C _D _E subG vs gl pg a)
          (ctlModelCheck _A _B _C _D _E subG vs gl pg b)
          (ctlModelCheck _A _B _C _D _E subG vs gl pg b)
    | subG, vs, gl, pg, EACtl a ->
        eaCheckAux
          (equal_list
            (equal_prod _B (equal_list (equal_suBigKWithBag _A _C _D))))
          (minus_nat, one_nat, zero_nat, equal_nat) (edgeNumsInGraph gl) pg
          (minusList
            (equal_list
              (equal_prod _B (equal_list (equal_suBigKWithBag _A _C _D))))
            vs (ctlModelCheck _A _B _C _D _E subG vs gl pg a))
          (ctlModelCheck _A _B _C _D _E subG vs gl pg a);;
...
let rec reversePairs = function [] -> []
                       | (a, b) :: l -> (b, a) :: reversePairs l;;

let rec getAllVertex _A
  = function [] -> []
    | (a, b) :: l -> inserta _A a (inserta _A b (getAllVertex _A l));;

let rec collectPairsAux a x1 = match a, x1 with a, [] -> []
                          | a, b :: l -> (a, b) :: collectPairsAux a l;;

let rec collectPairs = function [] -> []
                       | (a, al) :: l -> collectPairsAux a al @ collectPairs l;;

let rec getAllSubsortOnItem _A
  x0 a = match x0, a with [], a -> []
    | (x, y) :: l, a ->
        (if eq _A x a then y :: getAllSubsortOnItem _A l a
...
let rec reverseGraph _A
  g = formGraph _A (getAllVertex _A (reversePairs (collectPairs g)))
        (reversePairs (collectPairs g));;
...
let rec kctl_i_i_i_i_i_i _A _B _C _D _E
  x xa xb xc xd xe =
    sup_pred
      (bind (single (x, (xa, (xb, (xc, (xd, xe))))))
        (fun a ->
          (match a with (_, (_, (_, (_, (_, Success _))))) -> bot_pred
            | (theory, (_, (_, (_, (_, Failure xf))))) ->
              bind (eq_i_o (kcompile _A _B _D theory))
                (fun aa ->
                  (match aa with Success _ -> bot_pred
                    | Failure s ->
                      (if equal_lista equal_char xf
      ...
      (sup_pred
        (bind (single (x, (xa, (xb, (xc, (xd, xe))))))
          (fun a ->
            (match a with (_, (_, (_, (_, (_, Success _))))) -> bot_pred
              | (_, (_, (_, (_, (_, Failure []))))) -> bot_pred
              | (theory, (_, (l, (_, (_, Failure (ae :: lista)))))) ->
                case_char
                  (fun nibble1 nibble2 ->
...
| NibbleD -> bot_pred | NibbleE -> bot_pred | NibbleF -> bot_pred))
          ...
          (bind (single (x, (xa, (xb, (xc, (xd, xe))))))
            (fun a ->
              (match a
                with (theory, (n, (l, (names, (f, Success xf))))) ->
                  bind (eq_i_o (kcompile _A _B _D theory))
                    (fun aa ->
                      (match aa
                        with Success (theorya, (database, (subG, (_, _)))) ->
           ...
           (fun (transKRules, allKRules) ->
             bind (eq_i_o (divideAllBagRules noMacroRules))
               (fun (transBagRules, allBagRules) ->
                 bind (kctlAux_i_i_i_i_i_i_i_i_i_i_i_o _A _B _D database subG n
                        xaa xaaa allKRules allBagRules transKRules transBagRules
                        [(b, [b])] [b])
                   (fun xab ->
                     bind (eq_i_o (getCaresGraph _B names xab))
                       (fun xac ->
                         bind (eq_i_o
                                (getAllVertex
                                  (equal_list
                                    (equal_prod (equal_var _B)
                                      (equal_list
(equal_suBigKWithBag _A _B _D))))
                                  (collectPairs xac)))
                           (fun xaab ->
                             bind (eq_i_o
                                    (reverseGraph
                                      (equal_list
(equal_prod (equal_var _B) (equal_list (equal_suBigKWithBag _A _B _D))))
                                      xac))
                               (fun xaba ->
                                 bind (eq_i_o
(ctlModelCheck _A (equal_var _B) _B _D _E subG xaab xac xaba f))
                                   (fun xad ->
                                     bind (eq_i_o
    (findCellByNameList _B names b))
                                       (fun xaac ->
 (if equal_bool xf
       (membera
         (equal_list
           (equal_prod (equal_var _B)
             (equal_list (equal_suBigKWithBag _A _B _D))))
         xad xaac)
   then single () else bot_pred)))))))))))))
                            else bot_pred)
                        | Failure _ -> bot_pred))
                | (_, (_, (_, (_, (_, Failure _))))) -> bot_pred)))));;

let rec kctl _A _B _C _D _E
  x1 x2 x3 x4 x5 x6 =
    holds (kctl_i_i_i_i_i_i _A _B _C _D _E x1 x2 x3 x4 x5 x6);;
